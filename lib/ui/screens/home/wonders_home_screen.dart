...
class _WondersHomeScreenState extends State<WondersHomeScreen> with GetItStateMixin {
  ...
  void _handleSaveWallPaperPressed() async {
    // TODO: FIX, use off-screen widget instead
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> bgChildren = wonders
        .map(
          (e) => WonderIllustration(
            e.type,
            config: WonderIllustrationConfig.bg(isShowing: isSelected(e.type)),
          ),
        )
        .toList();

    List<Widget> mgChildren = wonders
        .map(
          (e) => WonderIllustration(
            e.type,
            config: WonderIllustrationConfig.mg(isShowing: isSelected(e.type)),
          ),
        )
        .toList();

    List<Widget> fgChildren = wonders
        .map(
          (e) => WonderIllustration(
            e.type,
            config: WonderIllustrationConfig.fg(isShowing: isSelected(e.type)),
          ),
        )
        .toList();
    ...
    return EightWaySwipeDetector(
      ...
      child: Stack(children: [
        ...
        BottomCenter(
          child: _AnimatedGradient(Colors.black.withOpacity(.6)),
        ),
